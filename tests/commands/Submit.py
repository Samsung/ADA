...
def test(app, equal):
	...
	app.message("submit rest1 rest4");
	
	delivery   = app.database.repos.deliveries.get_current();
	candidates = delivery.re_gathering.re_candidates;
	
	assert len(candidates) == 3;
	assert equal(candidates[0], "Candidate(id=2, gathering=1, restaurant=1, log=8)");
	assert equal(candidates[1], "Candidate(id=1, gathering=1, restaurant=3, log=7)");
	assert equal(candidates[2], "Candidate(id=3, gathering=1, restaurant=4, log=8)");
