...
class Reject(Base):
	...
	name   = "reject";
	help   = "Reject restaurant voting candidate for current delivery.";
	...
	def init(self):
		...
		self.add_argument("restaurants",
			...
			help  = "List of restaurants keys, that will be rejected from voting.");
	...
	def exec(self, whoami, author, channel, content, argv, args, log, user, delivery):
		...
		# Reject voting candidate
		candidates = self.services.gathering.rejects(gathering, restaurants);
